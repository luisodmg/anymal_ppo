...
class Agent(nn.Module):
    ...
    def __init__(self, obs_len, act_len, log_std_init=-0.5):
        ...
        self.actor  = nn.Sequential(nn.Linear(128, 128), nn.Tanh(), nn.Linear(128, act_len))
...
class Env():
    ...
    def step(self, raw_action, render=False):
        ...
        reward = (vel_reward + height_reward + upright_reward + alive_bonus
                  + action_cost + smooth_cost + joint_vel_cost)
...
def train(policy, optimizer, memory, hparams):
    ...
    for _ in range(ppo_epoch):
        for index in BatchSampler(SubsetRandomSampler(range(memory.buffer_capacity)),
                                  batch_size, False):
...
def render_episode(env, policy, episode, log_media=True, save_plot=True, prefix='video'):
...
def make_video(num_videos=5, policy_path=None, fall_threshold=0.3):
    ...
    for i_video in range(num_videos):
        ep_reward, _ = render_episode(env, policy, i_video + 1,
                                      log_media=False, save_plot=True, prefix='eval')
...
def train_or_sweep(is_sweep=True, live=False, overrides=None, run_name=None, num_episodes=None):
    """
    ...
    run = wandb.init(project=PROJECT, name=run_name, config=hparams,
    ...
    try:
        for i_episode in range(hparams['num_episodes']):
            ...
            if running_reward > saving_reward:
                ...
                policy_path = os.path.join(SAVE_DIR, f'{run_name}_{i_episode}_Reward-{running_reward}_policy.pt')
                optim_path  = os.path.join(SAVE_DIR, f'{run_name}_{i_episode}_Reward-{running_reward}_optimizer.pt')
                ...
                render_episode(env, policy, i_episode,
                               log_media=True, save_plot=True, prefix='checkpoint')
...
def run_sweep(count=50, episodes_per_trial=2000, live=False, seed=0):
    ...
    print(f"Starting random-search sweep: {count} trials x {episodes_per_trial} episodes each")
    for t in range(count):
        sampled = {k: _sample_param(v, rng)
                   for k, v in sweep_configuration["parameters"].items()}
        name = f"{sweep_configuration['name']}_trial{t:03d}"
        print(f"\n=== Trial {t + 1}/{count} :: {name} :: {sampled} ===")
        reward = train_or_sweep(is_sweep=True, live=live, overrides=sampled,
                                run_name=name, num_episodes=episodes_per_trial)
        if reward > best["reward"]:
            best = {"reward": reward, "name": name, "params": sampled}
        print(f"Trial {name} finished. reward={reward}  | best so far={best['reward']} ({best['name']})")
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        ...
        run_sweep(count=args.sweep_count, episodes_per_trial=args.sweep_episodes, live=args.live)
    elif args.mode == "render":
        make_video(num_videos=args.num_videos,
